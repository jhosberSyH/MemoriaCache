...
unsigned long int DirHexToDec(string direccionHex) {
    ...
    if (direccionHex.substr(0, 2) == "0x") {//Ignora los 2 primeros caracteres sin son "0x"
        direccionHex = direccionHex.substr(2);
    }

    unsigned long int decimal;
    istringstream(direccionHex) >> hex >> decimal;//Toma los hexadecimales restantes de la direccion de memoria y los transforma en decimal
